...
public class CandidatoDAO {
	...
	public Candidato[] modificarCandidato(String cedula, String nuevaCedula, String nuevoNombre, Candidato[] ArrayCandidato) {
		boolean existe = buscarCandidato(cedula, ArrayCandidato);
		if(existe == true){
			for(int i = 0; i < ArrayCandidato.length; i++) {
				if(ArrayCandidato[i]!= null){
	...
	public void mostrarArray(Candidato[] ArrayCandidato) {
		...
		for(int i = 0; i < ArrayCandidato.length; i++){
			if(ArrayCandidato[i]!=null) {
				...
				System.out.println("El candidato en la posicion "+ i + " no existe!");
			}
		}		
	}
